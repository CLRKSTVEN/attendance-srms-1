...
class Attendance extends CI_Controller {
    ...
    $old_debug = $this->db->db_debug; $this->db->db_debug = FALSE;
    ...
    if (!empty($op['ok']) && $op['ok'] && isset($op['mode']) && !empty($op['id'])) {
        ...
        if ($op['mode'] === 'checked_in') {
            $this->db->where('id', (int)$op['id'])
        ...
        } elseif ($op['mode'] === 'checked_out') {
            $this->db->where('id', (int)$op['id'])
                     ->group_start()->where('remarks IS NULL', null, false)->or_where('remarks','')->group_end()
                     ->set('remarks', 'Checked out via QR')->update('activity_attendance');
        }
    }
    ...
    public function profile()
{
    ...
    if ($this->db->table_exists('studentsignup')) {
        ...
        $courseCol = null; foreach ($courseCols as $c) { if (isset($fields[$c])) { $courseCol = $c; break; } }
        $majorCol  = null; foreach ($majorCols  as $m) { if (isset($fields[$m])) { $majorCol  = $m; break; } }
