...
.kpi:hover{transform:translateY(-2px);box-shadow:0 10px 26px rgba(36,59,83,.14)}
.kpi .card-body{display:flex;align-items:center;justify-content:space-between;padding:1.1rem 1.25rem}
.kpi .count{font-size:2.0rem;font-weight:800;color:#1f2d3d;margin:0;line-height:1}
.kpi .label{margin:.15rem 0 0;color:#546e7a;font-weight:700;letter-spacing:.2px}
.kpi .icon{width:56px;height:56px;border-radius:14px;display:grid;place-items:center;font-size:28px}

/* color accents per kpi */
.kpi.blue .icon { background:rgba(37,99,235,.08); color:#2563eb }
.kpi.pink .icon { background:rgba(236,72,153,.10); color:#ec4899 }
.kpi.purple .icon { background:rgba(139,92,246,.10); color:#8b5cf6 }
.kpi.cyan .icon { background:rgba(6,182,212,.10); color:#06b6d4 }
.kpi.primary .icon { background:rgba(59,130,246,.10); color:#3b82f6 }
...
.card.reg-ann{border:1px solid #dee2e6}
.card.reg-ann .card-header{background:#17a2b8;color:#fff;padding:.9rem 1rem}
.card.reg-ann .card-title{margin:0;font-weight:600}

.ann-row{border:1px solid #dee2e6;border-radius:8px;padding:1.0rem;background:#fdfdfd;box-shadow:0 3px 10px rgba(0,0,0,.05);transition:transform .25s ease,box-shadow .25s ease;margin-bottom:12px}
.ann-row:hover{transform:scale(1.01);box-shadow:0 6px 18px rgba(0,0,0,.1)}
.ann-title{font-size:1.05rem;font-weight:800;color:#2c3e50;margin-bottom:.25rem;border-left:5px solid #007bff;padding-left:.5rem}
.ann-meta{font-size:.9rem;color:#6c757d;margin-bottom:.5rem}
.ann-actions a{font-weight:600;color:#007bff;text-decoration:none}
...
.ann-text{flex:1;font-size:1rem;line-height:1.6;max-height:60vh;overflow:auto}
.ann-aside{width:38%;min-width:260px}
.ann-aside img{width:100%;height:auto;border-radius:8px;object-fit:contain}
@media (max-width:768px){
  .ann-flex{flex-direction:column}
  .ann-aside{width:100%;min-width:0}
  .ann-text{max-height:none}
}
...
.page-title-box .page-title{
  white-space: normal !important;   /* kill theme's nowrap */
  overflow: visible !important;      /* kill ellipsis clipping */
...
.kpi-grid{
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); /* desktop/tablet default */
  gap: 16px;
}

@media (max-width: 767.98px){
  .kpi-grid{
    grid-template-columns: repeat(2, minmax(0,1fr)); /* always 2-up on phones */
    gap: 12px;
  }
  /* Move the first card (Students' Profile) to LAST on phones */
  .kpi-grid > a:nth-child(1){ order: 5; }

  /* Tighter card visuals for small screens */
  .kpi .card-body{ padding: .9rem; }
  .kpi .count{ font-size: 1.6rem; }
  .kpi .icon{ width: 44px; height: 44px; font-size: 22px; border-radius: 12px; }
}
...
.card.enroll-card { border:1px solid #dee2e6; }
.card.enroll-card .card-header { background:#6f42c1; color:#fff; padding:.9rem 1rem; }
.card.enroll-card .card-title { margin:0; font-weight:600; }
.card.enroll-card .badge-term { background:#5a32a6; }
.card.enroll-card .table thead th { background:#f8f9fc; }

/* responsive stack for the two summary tables */
.enroll-split { display:flex; gap:24px; align-items:flex-start; }
.enroll-col { flex:1 1 0; min-width: 260px; }
@media (max-width: 991.98px){ .enroll-split{ flex-direction:column; gap:16px; } }
/* Make selected KPI span both columns on phones */
@media (max-width: 767.98px){
  .kpi-grid > a.kpi-span-2{ 
    grid-column: 1 / -1;      /* occupy both columns */
    order: 99;                 /* keep it at the bottom */
  }
}
    ...
    <div class="card reg-ann">
      ...
      <div id="adminAnnouncements" class="collapse show">
        <div class="card-body">
          ...
          <?php else: ?>
            <?php $i=0; foreach ($announcements as $row): $i++;
              $modalID  = 'annView'.$i;
