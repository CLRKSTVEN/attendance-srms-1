...
<style>
  ...
  .ann-row img{border:1px solid #ddd;background:#fff;padding:4px;margin-bottom:.5rem}
  .ann-row{border:1px solid #dee2e6;border-radius:8px;padding:1.5rem;background:#fdfdfd;box-shadow:0 3px 10px rgba(0,0,0,.05);transition:transform .25s ease,box-shadow .25s ease}
  .ann-row:hover{transform:scale(1.01);box-shadow:0 6px 18px rgba(0,0,0,.1)}
  .ann-title{font-size:1.25rem;font-weight:700;color:#2c3e50;margin-bottom:.5rem;border-left:5px solid #007bff;padding-left:.75rem}
  .ann-meta{font-size:.9rem;color:#6c757d;margin-bottom:.75rem}
  ...
  .ann-text{flex:1;font-size:1rem;line-height:1.6;max-height:60vh;overflow:auto;white-space:pre-wrap}
  ...
  /* Welcome hero */
  .welcome-hero{display:flex;align-items:center;justify-content:space-between;background:radial-gradient(110% 140% at 10% 10%,#e8f1ff 0%,#f3f7ff 35%,#ffffff 100%);border:1px solid #e6ecf5;border-radius:14px;padding:18px 20px;box-shadow:0 6px 18px rgba(66,133,244,.06)}
  .wh-kicker{font-size:.85rem;letter-spacing:.06em;text-transform:uppercase;color:#6b7b93}
  .wh-title{font-weight:800;color:#243b53}
  .wh-avatar{width:64px;height:64px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:1.4rem;color:#1b3a57;background:linear-gradient(135deg,#90caf9 0%,#42a5f5 50%,#1e88e5 100%);box-shadow:0 8px 18px rgba(66,133,244,.25)}

  /* Quick actions */
  .quick-actions .qa-card{display:flex;flex-direction:column;align-items:center;justify-content:center;background:#fff;border:1px solid #e6ecf5;border-radius:14px;padding:16px 10px;text-decoration:none;transition:transform .18s ease,box-shadow .18s ease,border .18s ease}
  .quick-actions .qa-card:hover{transform:translateY(-2px);box-shadow:0 10px 22px rgba(36,59,83,.12);border-color:#cfe3ff}
  .qa-icon{width:46px;height:46px;border-radius:12px;display:flex;align-items:center;justify-content:center;margin-bottom:8px;background:linear-gradient(135deg,#e3f2fd 0%,#e8f0fe 100%);font-size:1.25rem;color:#1e88e5}
  .qa-label{font-weight:600;color:#2c3e50}
...
$studNo = (string)$this->session->userdata('username'); // e.g., 176323811
...
if (!$displayName && $studNo) {
    $candidates = [
        ...
        ['table' => 'o_users',         'id_col' => 'username',      'select' => 'fName AS f, mName AS m, lName AS l'], // your login table
    ];
    ...
    foreach ($candidates as $c) {
        if ($this->db->table_exists($c['table'])) {
            ...
            if ($row) {
                ...
                // Format: Last, First Middle  (skip blanks gracefully)
                $name = ($l !== '' ? $l : '');
                if ($f !== '') $name .= ($name !== '' ? ', ' : '') . $f;
                if ($m !== '') $name .= ' ' . $m;
...
<?php if (isset($is_flagged) && $is_flagged && isset($flag_details)): ?>
  ...
  <style>
    @keyframes subtleFlash{0%,100%{background-color:#fff8e1;color:#856404}50%{background-color:#ffeeba;color:#856404}}
  </style>
