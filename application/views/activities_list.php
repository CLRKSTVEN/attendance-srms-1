...
<style>
.page-title-box{position:relative;z-index:0;}
  .actions{display:flex;justify-content:center;gap:.35rem;flex-wrap:wrap}
  .btn-icon{position:relative;width:40px;height:40px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #ced4da;border-radius:10px;background:#fff;transition:all .15s ease}
  .btn-icon:hover{box-shadow:0 3px 10px rgba(0,0,0,.08);transform:translateY(-1px)}
  .btn-icon i{font-size:18px;line-height:1}
  .btn-icon .hint{position:absolute;bottom:110%;left:50%;transform:translate(-50%,4px);background:#343a40;color:#fff;font-size:11px;font-weight:600;letter-spacing:.3px;padding:2px 6px;border-radius:4px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .15s ease,transform .15s ease}
  .btn-icon .hint::after{content:"";position:absolute;top:100%;left:50%;transform:translateX(-50%);border:5px solid transparent;border-top-color:#343a40}
  .btn-icon:hover .hint{opacity:1;transform:translate(-50%,0)}
  .btn-scan{border-color:#6c757d}.btn-scan i{color:#6c757d}
  .btn-poster{border-color:#0d6efd}.btn-poster i{color:#0d6efd}
  .btn-delete{border-color:#dc3545}.btn-delete i{color:#dc3545}
  .btn-edit{border-color:#198754}.btn-edit i{color:#198754}
  .table thead th{white-space:nowrap}
  .date-icons{max-width:180px;white-space:nowrap}
  .date-icons .ic{
    width:22px;height:22px;display:inline-flex;align-items:center;justify-content:center;
    border:1px solid #e5e7eb;border-radius:999px;background:#fff;color:#374151;margin-right:6px;vertical-align:middle;
  }
  .date-icons .ic i{font-size:13px;line-height:1}
  .date-icons .ic.cal{width:26px}
  .date-icons .ic.am{background:#e8f4ff;border-color:#cfe3ff;color:#0b5ed7}
  .date-icons .ic.pm{background:#fff3cd;border-color:#ffe69c;color:#946200}
  .date-icons .ic.eve{background:#efe7ff;border-color:#d6c9ff;color:#5a3bd6}
  ...
  @media (max-width: 767.98px){
    ...
    .table.resp-table tr{display:block;border:1px solid #e9ecef;border-radius:12px;margin:.5rem 0;box-shadow:0 2px 10px rgba(15,23,42,.05)}
    .table.resp-table td{display:flex;justify-content:space-between;align-items:flex-start;gap:.75rem;padding:.6rem .75rem;border:0;border-bottom:1px dashed #e9ecef;white-space:normal;word-break:break-word}
    .table.resp-table td:last-child{border-bottom:0}
    .table.resp-table td[data-label]::before{content:attr(data-label);flex:0 0 44%;font-weight:600;color:#6b7280}
    .actions{justify-content:flex-start}
    .btn-icon{width:44px;height:44px}
    .date-icons{white-space:normal}
  }
  @media (max-width: 767.98px){
    #logTable{display:block;width:100%}
    #logTable thead{display:none}
    #logTable tbody{display:block}
    #logTable tr{display:block;border:1px solid #e9ecef;border-radius:12px;margin:.5rem 0;box-shadow:0 2px 10px rgba(15,23,42,.05)}
    #logTable td{display:flex;justify-content:space-between;align-items:flex-start;gap:.7rem;padding:.55rem .7rem;border:0;border-bottom:1px dashed #e9ecef;white-space:normal;word-break:break-word}
    #logTable td:last-child{border-bottom:0}
    #logTable td[data-label]::before{content:attr(data-label);flex:0 0 44%;font-weight:600;color:#6b7280}
  }
  @media print{
    ...
    tr{display:table-row !important;box-shadow:none !important;border:none !important;margin:0 !important}
    td,th{display:table-cell !important;white-space:normal !important;word-break:break-word !important;border:1px solid #dee2e6 !important;padding:.5rem !important;vertical-align:middle !important}
    td::before{content:none !important}
  }
  .row-flash{animation:rowFlash 1.2s ease-out 1}
  @keyframes rowFlash{0%{background:#e6fffa}100%{background:transparent}}
  .src-badge{display:inline-block;padding:3px 8px;border-radius:999px;font-weight:600;font-size:.75rem}
  .src-qr{background:#e6f2ff;color:#1d4ed8;border:1px solid #bfdbfe}
  .src-man{background:#fef3c7;color:#92400e;border:1px solid #fde68a}
@media (max-width: 767.98px){
  .table.resp-table td{
    min-width: 0; 
  }
...
@media (max-width: 767.98px){
  ...
  .table.resp-table td > *{ grid-column: 2; min-width: 0; max-width: 100%; }
  td[data-label="Program"] .badge{
    display:inline-block;
    white-space: normal !important;
    word-break: break-word;
  ...
  td[data-label="Date"] .date-icons{ white-space: normal; }
  td[data-label="Date"] .date-icons .ic{ margin: 0 6px 6px 0; }
...
@media (max-width: 767.98px){
  ...
  #logTable td > *{ grid-column: 2; min-width: 0; max-width: 100%; }
}
...
<?php endforeach; else: ?>
  <tr><td colspan="6" class="text-center text-muted py-4">No activities yet.</td></tr>
<?php endif; ?>
                </tbody>
              </table>
            </div>
...
<script>
  ...
  (function () {
    let _logActivityId = null, _autoTimer = null;
    ...
    function showSpinner(s){ if (spinnerEl) spinnerEl.style.display = s ? 'flex':'none'; }
    ...
    function fmt(iso){
      if (!iso) return '';
      try { return moment(iso).format('h:mm:ss A'); } // 12-hour
      catch(e){ return iso; }
    }
...
rows.sort((a, b) => {
    ...
    const rank = s => ({ am: 1, morning: 1, pm: 2, afternoon: 2, eve: 3, evening: 3 }[String(s || '').toLowerCase()] || 9);
  ...
  '<td data-label="Remarks">'+ remarkFor(row) +'</td>';
          ...
          logTableEl
            ...
            .on('click.copy','td.copy-sn', function(){
              ...
              $(this).addClass('row-flash'); setTimeout(()=>$(this).removeClass('row-flash'), 800);
            });
    ...
    $('#logModal')
      ...
      .on('hidden.bs.modal', function () { setAuto(false); });
  })();
</script>
</body>
</html>
